...
bool Sh2PlatSort::load(char* shm_name)
{
	...
	if (!market->load(sizeof(STOCKSNAP2), sizeof(STOCKLINE2), sizeof(STOCKTRADE2)))
	{
...
void Sh2PlatSort::update()
{
	int i=0;
	STOCKSNAP2* pstocksnap;
	map<string, STOCKSNAP2> mSnap;

	while((pstocksnap = (STOCKSNAP2*)market->getSnap(++i))!=NULL)
	{
