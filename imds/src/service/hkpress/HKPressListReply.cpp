/*
 * HKPressListReply.cpp
 *
 *  Created on: Sep 24, 2015
 *      Author: level2
 */

#include "HKPressListReply.h"
#include "../../public/hkpress/HKPressSnap.h"

COL_DESC_ST HKPressListReply::HKPressListDesc[] =
{
...
HKPressListReply::HKPressListReply() {
...
void HKPressListReply::registerCol()
{
    if (this->hTblColDes == NULL)
    {
        HTableCreate(&this->hTblColDes);
        int n = sizeof(HKPressListDesc) / sizeof(COL_DESC_ST);
        for (int i = 0; i < n; i++)
        {
            COL_DESC_ST *description = &HKPressListDesc[i];
            HTableEnter(this->hTblColDes, description->key,
                    (char *) description, NULL);
        }
    }
}
int HKPressListReply::request(const char* script,const char* query,char* buffer,int& len)
{
	...
	date=((HKPSNAP*)list[0].snap)->date;
	time=((HKPSNAP*)list[0].snap)->time;
	return ListReply::reply(buffer,len);
}

bool HKPressListReply::getSelfList()
{
	list.clear();
	ListCmp cmp;
...
bool HKPressListReply::getTypeList()
{
	list.clear();
	ListCmp cmp;
	if ((order.find("ASE")!=string::npos) || (order.find("ase")!=string::npos))
...
bool HKPressListReply::isExchangeSubType(string& subType,char* snap)
{
	HKPSNAP* s=(HKPSNAP*)snap;
	/*
	if (listSubType.find("main") != string::npos)
		return strcmp(type, "MAIN") == 0;
	if (listSubType.find("gem") != string::npos)
		return strcmp(type, "GEM") == 0;
	if (listSubType.find("ets") != string::npos)
		return strcmp(type, "ETS") == 0;
	if (listSubType.find("nasdaq") != string::npos)
		return strcmp(type, "NASD") == 0;
	*/
	if (listSubType.find("bond")!=string::npos)
		return strcmp(s->basicinfo.securityType, "BOND") == 0;
	if (listSubType.find("basket_warrant")!=string::npos)
		return strcmp(s->basicinfo.securityType, "BWRT") == 0;
	if (listSubType.find("equity")!=string::npos)
		return strcmp(s->basicinfo.securityType, "EQTY") == 0;
	if (listSubType.find("trust")!=string::npos)
		return strcmp(s->basicinfo.securityType, "TRST") == 0;
	if (listSubType.find("warrant")!=string::npos)
		return strcmp(s->basicinfo.securityType, "WRNT") == 0;
	if (listSubType.find("main") != string::npos)
		return strcmp(s->marketID, "MAIN") == 0;
	if (listSubType.find("gem") != string::npos)
		return strcmp(s->marketID, "GEM") == 0;
	if (listSubType.find("ets") != string::npos)
		return strcmp(s->marketID, "ETS") == 0;
	if (listSubType.find("nasdaq_amx") != string::npos)
		return strcmp(s->marketID, "NASD") == 0;
	return false;
}

bool HKPressListReply::getOrder(ListCmp& cmp)
{
	HKPSNAP* s=(HKPSNAP*)cmp.snap;
	if (!order.empty())
	{
        if (order.find("change") != string::npos)
        {
            cmp.type = 1;
            if (s->lastPx > 0)
                cmp.iCmp = static_cast<int64_t>(s->lastPx - s->prevClosePx);
            else
                cmp.iCmp = 0;
            return true;
        }
        else if (order.find("chg_rate") != string::npos)
        {
            ...
            if (s->prevClosePx != 0 && s->lastPx > 0)
                cmp.dCmp = static_cast<int64_t>((s->lastPx - s->prevClosePx)) * 100.0
                        / s->prevClosePx;
            else
                cmp.dCmp = 0;
            return true;
        ...
        else if (order.find("amp_rate") != string::npos)
        {
            cmp.type = 3;
            if (s->prevClosePx != 0 && s->lastPx > 0)
                cmp.dCmp = static_cast<int64_t>((s->highPx - s->lowPx)) * 100.0
                        / s->prevClosePx;
            else
                cmp.dCmp = 0;
            return true;
		...
		else if (order.find("nominal") != string::npos)
		{
			cmp.type = 1;
			cmp.iCmp = s->nominalpx;
			return true;
		}
		...
		{
			cmp.type=2;
			cmp.lCmp=s->bid.price;
			return true;
		}
		else if (order.find("askpx1")!=string::npos)
		{
			cmp.type=2;
			cmp.lCmp=s->offer.price;
			return true;
		}
...
int HKPressListReply::getSnapString(char* buffer,const char* select)
{
	HKPSNAP* s=(HKPSNAP*)curSnap;
	...
	if (strcmp(select,"change")==0)
	{
		if (s->lastPx!=0)
		{
			return sprintf(buffer,"%s",getFloat(static_cast<int64_t>(s->lastPx-s->prevClosePx)));
		}
		else
			return sprintf(buffer,"0.000");
	}
	...
	{
		if (s->prevClosePx != 0 && s->lastPx>0)
			return sprintf(buffer,"%.2f",static_cast<int64_t>((s->lastPx-s->prevClosePx))*100.0/s->prevClosePx);
		else
			return sprintf(buffer,"0.00");
	}
	if (strcmp(select,"amp_rate")==0)
	{
		if (s->prevClosePx!=0 && s->lastPx>0)
			return sprintf(buffer,"%.2f",static_cast<int64_t>((s->highPx-s->lowPx))*100.0/s->prevClosePx);
		else
			return sprintf(buffer,"0.00");
	}
	...
	if (strcmp(select,"bidpx1")==0)
		return sprintf(buffer,"%s",getHKFloat(s->bid.price));
	if (strcmp(select,"askpx1")==0)
		return sprintf(buffer,"%s",getHKFloat(s->offer.price));
	return 0;
	*/
}
