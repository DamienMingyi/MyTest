...
#include "Stock2ListReply.h"
#include "../../public/stock2/StockSnap2.h"

COL_DESC_ST Stock2ListReply::stock2ListDesc[] =
{
...
Stock2ListReply::Stock2ListReply() {
...
void Stock2ListReply::registerCol()
{
    if (this->hTblColDes == NULL)
    {
        HTableCreate(&this->hTblColDes);
        int n = sizeof(stock2ListDesc) / sizeof(COL_DESC_ST);
        for (int i = 0; i < n; i++)
        {
            COL_DESC_ST *description = &stock2ListDesc[i];
...
int Stock2ListReply::request(const char* script,const char* query,char* buffer,int& len)
{
	...
	date=((STOCKSNAP2*)list[0].snap)->date;
	time=((STOCKSNAP2*)list[0].snap)->time;
	return ListReply::reply(buffer,len);
}

bool Stock2ListReply::getSelfList()
{
	list.clear();
	ListCmp cmp;
...
bool Stock2ListReply::getTypeList()
{
    list.clear();
    ListCmp cmp;
    if ((order.find("ASE") != string::npos) || (order.find("ase") != string::npos))
    ...
    if (listType.find("exchange") != string::npos)
    {
        ...
        for (int i = 1; i < st; ++i)
        {
            ...
            if (cmp.snap == NULL)
                break;
            int id = atoi(((STOCKSNAP2*) cmp.snap)->securityID);
            bool isList = false;
            if (listSubType.find("all") != string::npos)
            {
                isList = true;
            }
            else if (listSubType.find("ashare") != string::npos)
            {
                isList = (id >= 600000 && id <= 609999);
            }
            else if (listSubType.find("bshare") != string::npos)
            {
                isList = (id >= 900000 && id <= 909999);
            }
            else if (listSubType.find("fund") != string::npos)
            {
                isList = (id >= 500000 && id <= 510999);
            }
            else if (listSubType.find("bond") != string::npos)
            {
                isList = (id >= 9000 && id <= 299999);
            }
            else if (listSubType.find("index") != string::npos)
            {
                isList = (id <= 999);
            }
            if (isList)
...
bool Stock2ListReply::getOrder(ListCmp& cmp)
{
	STOCKSNAP2* s=(STOCKSNAP2*)cmp.snap;
	if (!order.empty())
	{
        if (order.find("change") != string::npos)
        {
            cmp.type = 1;
            if (s->lastPx > 0)
                cmp.iCmp = s->lastPx - s->prevClosePx;
            else
                cmp.iCmp = 0;
            return true;
        }
		...
		else if(order.find("close") != string::npos)
		{
			cmp.type = 1;
			cmp.iCmp = s->prevClosePx;
			return true;
...
int  Stock2ListReply::getSnapString(char* buffer,const char* select)
{
	STOCKSNAP2* s=(STOCKSNAP2*)curSnap;
