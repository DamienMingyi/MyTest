...
bool Sh1Reference::initial(const char* referencePath, const char* shm)
{
	time_t tt = time(NULL);
	...
	StockSnap stocksnap;
	Market market("sh1",&shmEnv,&stocksnap);
	...
	vStock[0].amount=vStock.size();
	market.initSnap(0,(char*)&vStock[0]);
	market.refresh();
	return true;
}
bool Sh1Reference::fjyStaticParse(char* path)
{
	...
	STOCKSNAP stock;
	memset(&stock, 0, sizeof(STOCKSNAP));
...
bool Sh1Reference::mktdt00StaticParse(char* path)
{
	STOCKSNAP stock;
	int32_t length = getFileBufferLength(path);
	if (length == -1)
...
bool Sh1Reference::plateFileParse(char* path,const char flag)
{
	STOCKSNAP stock;
	int32_t length = getFileBufferLength(path);
	if (length == -1)
...
void Sh1Reference::getmGnpz(char *conceptcode , char* securities)
{
	char code[16] = {0};
