...
#include "Sh1Process.h"

Sh1Process::Sh1Process(Profile* pf, IDataHandle* dh): Process(pf, dh) {
	// TODO Auto-generated constructor stub

}

Sh1Process::~Sh1Process() {
	// TODO Auto-generated destructor stub
	if (dh != NULL)
		delete dh;
}

int32_t Sh1Process::main(int argc, char* argv[])
{
	...
	else if (argc == 2 && strcmp(argv[1], "kill") == 0)
	{
		...
		{
			kill(atoi(v), SIGTERM);
			INFO("sh1transfer(%s) be killed", v);
			return 0;
		}
		else
		{
			INFO("sh1transfer not run");
			return 1;
		}
	}
	else if (argc == 1)
	...
	else
	{
		...
		ERROR("sh1transfer tcp: sse_template_path ip port shm_path mode security_type msg_type upstream");
		ERROR("sh1transfer file: sse_template_path rawdata_path shm_path mode security_type msg_type upstream");
		ERROR("sh1transfer udp: sse_template_path localip ip port shm_path mode security_type msg_type upstream");
		return 1;
	}
	if (!pf->setPid(true,true))
	{
		ERROR("sh1transfer already run");
		return 1;
	}

	if (port && localIP[0] == '\0')
	{
		INFO("sh1transfer %s %s %d %s %d %s %s %s", sse_template_path, ip, port, shm_path,
				mode, securityType, msgType, upstream);
		stream = new TCPSocketStream(ip, port);
	}
	else if (localIP[0] == '\0')
	{
		INFO("sh1transfer %s %s %s %d %s %s %s", sse_template_path, data_path, shm_path,
				mode, securityType, msgType, upstream);
		stream = new FileStream(data_path);
	}
	else if (port && localIP[0] != '\0')
	{
		INFO("sh1transfer %s %s %s %d %s %d %s %s %s", sse_template_path, localIP, ip, port,
