...
void MD5::MD5_Key(char* buff, int buffLen, char* md5key)
{
    unsigned char digest[16] =  { 0 };

    if (!buff || buffLen <= 0 || !md5key)
    {
        return;
    }
    ...
    Init();
    Update((unsigned char*) buff, buffLen);
    Final(digest);
    for (int i = 0; i < 16; i++)
    {
        sprintf(&md5key[i * 2], "%02x", digest[i]);
    }
    md5key[32] = 0;
}
...
**修订日期	修订人	修复的bug或修订原因
******************************************************************/
void MD5::MD5_String(char* msg,char* md5key)
{
    ...
    Init();
    Update((unsigned char*) msg, msglen);
    Final(digest);
    for (int i = 0; i < 16; i++)
    {
        sprintf(&md5key[i * 2], "%02x", digest[i]);
...
**修订日期	修订人	修复的bug或修订原因
******************************************************************/
void MD5::MD5_File(const char* filename,char* md5key)
{
    unsigned char digest[16];
    memset(digest, 0, sizeof(digest));
    memset(md5key, 0, 32);
