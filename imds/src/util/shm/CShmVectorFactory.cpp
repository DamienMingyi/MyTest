...
/**
 * CShmVectorFactory构造函数
 */
CShmVectorFactory::CShmVectorFactory(IAlloctor *ptrAlloctor)
...
/**
 * CShmVectorFactory析构函数
 */
CShmVectorFactory::~CShmVectorFactory()
{
    // TODO Auto-generated destructor stub
}

shm_status_t CShmVectorFactory::addObjToHt(const char* shmObjName,
        shm_offset_t value)
{
...
CShmObj* CShmVectorFactory::createShmObj(const char* shmObjName,
        shm_size_t itemlen, shm_size_t capacity)
{
    ...
    shm_size_t singlePage = (pageSize * itemlen) + sizeof(ST_SHM_VECTOR_PAGE);
    ...
    ptrObjProperty->shmObjType = SHM_OBJ_VECTOR;
    ...
    vectorProperty->itemLen = itemlen;
    ...
    ptrChunkReader->unLock();
    //根据属性实例化容器
    CShmVector* shmObj = new CShmVector(ptrStaticAlloctor, ptrChunkReader,
            objPropertyOffset);
    return shmObj;
}

CShmObj* CShmVectorFactory::openShmObj(const char* shmObjName)
{
    CStaticAlloctor *ptrStaticAlloctor = (CStaticAlloctor *) ptrAlloctor;
    CChunk *ptrChunk = (CChunk *) ptrStaticAlloctor->getChunk();
    CChunkReader *ptrChunkReader = (CChunkReader *) ptrChunk->getReader();
    ...
    if (SHM_OBJ_VECTOR != ptrObjProperty->shmObjType)
    {
        //名字能对上,但类型不对
        return NULL;
    }
...
CShmObj* CShmVectorFactory::getShmObj(const char* shmObjName,
        shm_size_t itemlen, shm_size_t capacity)
{
    //参数检查
    if (shmObjName == NULL)
