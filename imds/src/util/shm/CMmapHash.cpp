 ...
 *  CMmapHash构造函数
 */
CMmapHash::CMmapHash(ST_SHM_HT *ptrShmDictHt)
{
    ptrHt = ptrShmDictHt;
}
 ...
 *  - false 失败
 */
bool CMmapHash::create(shm_u32_t size)
{
    if (ptrHt)
    {
        //至少有一个节点
 ...
 *  - false 失败
 */
bool CMmapHash::enter(const shm_u32_t key, shm_offset_t value)
{
    //参数检查
    if (!ptrHt)
    {
 ...
 *  - false 失败
 */
bool CMmapHash::enter(const char *key, shm_offset_t value)
{
    //参数检查
    if (!ptrHt)
    {
 ...
 *  - 大于0 查找结果
 */
shm_offset_t CMmapHash::find(const char *key)
{
    if (!ptrHt)
    {
        return 0;
 ...
 *  - 大于0 查找结果
 */
shm_offset_t CMmapHash::find(const shm_u32_t key)
{
    if (!ptrHt)
    {
        return 0;
 ...
 *  - false 失败
 */
bool CMmapHash::list(DICT_ENTRY_LIST &dictEntryList)
{
    dictEntryList.clear();
    if (!ptrHt)
    {
